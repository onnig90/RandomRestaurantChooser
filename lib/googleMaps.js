...
async function findPlaceByNameAndLocation(name, lat, lng) {
  if (!name || !Number.isFinite(lat) || !Number.isFinite(lng) || !hasServerApiKey()) {
    return null;
  }

  const data = await fetchGoogleJson(`${GOOGLE_PLACES_BASE_URL}/places:searchText`, {
    method: 'POST',
    fieldMask: ['places.id', 'places.displayName', 'places.location'].join(','),
    body: {
      textQuery: name,
      maxResultCount: 5,
      locationBias: {
  ...
  // Pick the closest match to the given coordinates.
  let best = null;
  let bestDistance = Infinity;
  for (const place of places) {
    const placeLat = place.location && place.location.latitude;
    const placeLng = place.location && place.location.longitude;
    if (!Number.isFinite(placeLat) || !Number.isFinite(placeLng)) continue;

    const distance = Math.hypot(placeLat - lat, placeLng - lng);
  ...
  return best && best.id ? normalizePlaceId(best.id) : null;
}
