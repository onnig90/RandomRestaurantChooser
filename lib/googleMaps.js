...
async function searchNearbyRestaurants(location, options = {}) {
  ...
  if (options.primaryType) {
    includedPrimaryTypes.push(options.primaryType);
  } else {
    includedPrimaryTypes.push('restaurant');
  }
  ...
  const body = {
    ...
    locationRestriction: {
      circle: {
        ...
        radius: Math.max(1, Math.min(options.radius || 5000, 50000)),
      },
    },
  };

  const data = await fetchGoogleJson(`${GOOGLE_PLACES_BASE_URL}/places:searchNearby`, {
    method: 'POST',
    fieldMask: [
      'places.id',
      'places.displayName',
