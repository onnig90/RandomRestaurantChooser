...
async function applyRouteDistanceFilter(location, restaurants, maxDistanceMeters) {
  ...
  try {
    ...
    return restaurants.filter((r) => r.distanceMeters <= maxDistanceMeters);
  }

  return restaurants
    .map((r, i) => {
      const routeDistance = routeDistances[i];
      return {
        ...r,
        routeDistanceMeters: Number.isFinite(routeDistance) ? routeDistance : null,
      };
    })
    .filter((r) => {
      // If the route lookup failed for this specific point, fall back to Haversine
      // so we never accidentally admit something above the hard limit.
      const effectiveDistance = r.routeDistanceMeters !== null ? r.routeDistanceMeters : r.distanceMeters;
      return effectiveDistance <= maxDistanceMeters;
    });
}

function mergeSources(googleResults, osmResults) {
  const merged = [...googleResults];
  const seenKeys = new Set(googleResults.map((r) => normalizeRestaurantName(r.name)));

  for (const osmResult of osmResults) {
    const key = normalizeRestaurantName(osmResult.name);
    if (key && !seenKeys.has(key)) {
      seenKeys.add(key);
      merged.push(osmResult);
    }
  }
...
class RestaurantDAO {
  ...
  async GetRestaurants(location, filter) {
    ...
    if (cachedEntry) return cachedEntry.restaurants;

    // Run both sources in parallel — Google for quality data,
    // OSM for complete geographic coverage with no result cap.
    const [googleResult, osmResult] = await Promise.allSettled([
      this.fetchGooglePlacesRaw(location, filter),
      this.fetchOSMRaw(location, filter),
    ]);

    const googleRaw = googleResult.status === 'fulfilled' ? googleResult.value : [];
    const osmRaw = osmResult.status === 'fulfilled' ? osmResult.value : [];

    if (googleRaw.length === 0 && osmRaw.length === 0) {
      const err = new Error('Restaurant search is temporarily unavailable');
      err.status = 502;
      throw err;
    }

    // Google data takes precedence; OSM fills every gap inside the radius.
    const merged = mergeSources(googleRaw, osmRaw);

    // Apply Haversine distance, cuisine, price, open-hours filters.
    const candidates = finalizeRestaurants(location, merged, filter);

    // Apply actual driving-distance filter (skipped for large sets).
    const restaurants = await applyRouteDistanceFilter(
      location,
      candidates,
  ...
  async fetchGooglePlacesRaw(location, filter) {
    ...
    return rawPlaces.map(mapGooglePlaceToRestaurant).filter(Boolean);
  }

  async fetchOSMRaw(location, filter) {
    const { lat, lng } = location;
    const radius = Math.max(500, Math.min(filter.maxDistance || 5000, 25000));
...
`;
    ...
    return (data.elements || []).map(osmElementToRestaurant).filter(Boolean);
  }
}
